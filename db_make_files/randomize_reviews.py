...
def generate_random_reviews_with_delays(num_reviews, database_location='activity_recommendations.db'):
    ...
    unique_reviews = set()  # Set to store unique composite IDs

    # Generate random reviews with delays
    while len(unique_reviews) < num_reviews:
        ...
        # Check if the compositeID is already in the set
        if composite_id in unique_reviews:
            continue  # Skip this iteration and generate another review

        # Add the compositeID to the set
        unique_reviews.add(composite_id)
